...
const scoreboard = {
  ...
  overall,
  categories: {
    contractBoundaryIntegrity: results.find((r)=>r.command.includes('status-truth'))?.status ?? 'unknown',
    threatSeamStatus: results.find((r)=>r.command.includes('threat-seams'))?.status ?? 'unknown',
    versionDriftStatus: results.find((r)=>r.command.includes('version-drift'))?.status ?? 'unknown',
  },
  verification: results,
};
mkdirSync('artifacts/governance', { recursive: true });
writeFileSync('artifacts/governance/governance-scoreboard.json', JSON.stringify(scoreboard, null, 2) + '\n');
