...
const components: Component[] = [
  {
    ...
    implementationEvidence: ['src/lib/control-plane/orchestration.ts', 'src/lib/control-plane/scheduler.ts'],
    testEvidence: ['src/lib/control-plane/control-plane.test.ts', 'src/lib/control-plane/runtime-seams.test.ts'],
  ...
  },
  {
    ...
    state: hasFile(/scripts\/verify-proofpack\.ts$/) && hasFile(/scripts\/verify-replay\.ts$/) ? 'implemented' : 'partial',
    implementationEvidence: ['src/lib/control-plane/proofpack-ledger.ts', 'src/lib/control-plane/replay.ts'],
...
for (const component of components) {
  for (const path of [...component.implementationEvidence, ...component.testEvidence, ...component.docsEvidence]) {
...
if (/target state/i.test(docs.target) && /planned/i.test(docs.target) && components.some((c) => c.state === 'implemented')) {
...
const conflicting = components.filter(
  (c) => c.state === 'implemented' && (c.testEvidence.length === 0 || c.implementationEvidence.length === 0),
);
...
writeFileSync('artifacts/governance/status-truth-map.json', `${JSON.stringify(payload, null, 2)}\n`);

if (staleClaims.length > 0 || conflicting.length > 0 || components.some((c) => c.driftFindings.length > 0)) {
