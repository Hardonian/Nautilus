...
type Pkg = { version: string; dependencies?: Record<string,string>; devDependencies?: Record<string,string> };
...
const issues: string[] = [];
const allowlist = JSON.parse(readFileSync('docs/verification/version-drift-allowlist.json','utf8')) as {allowed: Allow[]};
const isAllowed=(key:string,rootV:string,nestedV:string)=>allowlist.allowed.some((a)=>a.key===key&&a.root===rootV&&a.nemoclaw===nestedV);
...
for (const dep of ['typescript','vitest']) {
  ...
  if (a && b && a !== b && !isAllowed(`devDependency:${dep}`, a, b)) issues.push(`Tooling drift for ${dep}: root=${a} nemoclaw=${b}`);
}
