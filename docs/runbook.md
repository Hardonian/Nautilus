...
```bash
# Docker
docker tag nautilus:previous nautilus:latest
docker compose up -d --force-recreate

# Kubernetes
kubectl rollout undo deployment/nautilus
```

### Logs
```bash
# Structured JSON logs
docker logs -f nautilus | jq .

# Filter by trace_id
docker logs nautilus | grep "trace_id=\"abc123\""
```
