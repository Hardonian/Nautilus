...
| R1 hardening utilities | Added deterministic unit coverage for bounded retry, trust boundary deny-by-default, unavailable degraded helper, cleanup idempotence | Broader integration coverage in remote execution seams | `vitest run src/lib/control-plane/r1-r4-r5-implementation.test.ts` | Hardening regressions could silently weaken fail-closed behavior |
| R4 handoff/context primitives | Added deterministic unit coverage for packet validation, evidence-preserving compaction, fingerprint reuse, token overflow degraded branch, checkpoint replay | Cross-module integration with orchestration pipeline | `vitest run src/lib/control-plane/r1-r4-r5-implementation.test.ts` | Long-context/handoff flows may drift from deterministic contracts |
| R5 queue/proofpack/release gate | Added unit coverage for capacity load-shed, idempotency dedupe, retry dead-letter, proofpack composition, release-waiver gate, benchmark unavailable branch, verification script existence checks | Integration pipeline across proofpack/replay/export still pending | `vitest run src/lib/control-plane/r1-r4-r5-implementation.test.ts` | Queue governance and release controls may regress without explicit checks |
